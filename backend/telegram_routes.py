...
_STAGING_DIR = Path("/var/lib/aihomecloud/telegram_setup_staging")


async def _install_binary_from(source_path: Path) -> tuple[bool, str]:
    """Stage *source_path* (no privilege needed -- both ends are within this
    service's own ReadWritePaths=) then ask systemd to run an unsandboxed
    oneshot unit, in the host namespace, that copies it to _BINARY_PATH and
    chmods it -- authorized via a scoped polkit rule, not sudo (which
    NoNewPrivileges=yes on this service blocks outright)."""
    _STAGING_DIR.mkdir(parents=True, exist_ok=True)
    staged = _STAGING_DIR / "telegram-bot-api"
    shutil.copy(source_path, staged)
...
async def _install_service_unit(content: str) -> tuple[bool, str]:
    """Same pattern as _install_binary_from(), for the systemd unit file
    itself (destination /etc/systemd/system/, also root-only)."""
    _STAGING_DIR.mkdir(parents=True, exist_ok=True)
    staged = _STAGING_DIR / f"{_SERVICE_NAME}.service"

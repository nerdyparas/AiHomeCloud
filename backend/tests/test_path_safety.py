...
@pytest.mark.asyncio
async def test_valid_subdirectory_path_returns_200(client: AsyncClient):
    """
    7B.3: GET /api/v1/files/list?path=/srv/nas/valid/subdir returns 200
    
    Verify that legitimate paths within NAS root are allowed.
    """
    response = await client.get("/api/v1/files/list?path=/srv/nas/shared/")
...
@pytest.mark.asyncio
async def test_url_encoded_path_traversal_returns_403(client: AsyncClient):
    """
    ...
    response = await client.get("/api/v1/files/list?path=%2F..%2Fetc")
    assert response.status_code == 403, "URL-encoded path traversal should return 403"


@pytest.mark.asyncio
async def test_path_exceeding_4096_characters_returns_400(client: AsyncClient):
    """
    ...
    response = await client.get(f"/api/v1/files/list?path={long_path}")
...
@pytest.mark.asyncio
async def test_path_with_null_bytes_returns_403(client: AsyncClient):
    """
    Verify that paths containing null bytes (or URL-encoded %00) are rejected.
    """
    # %00 = null byte in URL encoding
    response = await client.get("/api/v1/files/list?path=/srv/nas/test%00/etc")
    ...
    try:
        ...
        if link_path.parent.exists():
            try:
                ...
                response = await client.get("/api/v1/files/list?path=/srv/nas/evil_link")
...
@pytest.mark.asyncio
async def test_double_encoding_traversal_attempt_returns_403(client: AsyncClient):
    """
    ...
    response = await client.get("/api/v1/files/list?path=%252F..%252Fetc")
...
@pytest.mark.asyncio
async def test_empty_path_defaults_to_shared(client: AsyncClient):
    """

...
class TestSetupLocalApi:
    ...
    async def test_install_binary_from_stages_and_starts_unit(self, tmp_path):
        """_install_binary_from() must stage the source file at the fixed
        staging path (real file I/O -- no privilege needed, both ends are
        under this service's own ReadWritePaths=) then ask systemd to run
        the host-namespace install unit, rather than ever calling
        cp/chmod as a direct subprocess (which NoNewPrivileges=yes blocks
        for sudo, and which would only affect this service's own private
        mount-namespace view even without sudo -- see storage_helpers.py's
        mount_nas_device() for the same reasoning applied to storage)."""
        from app.routes import telegram_routes

        source = tmp_path / "downloaded-binary"
        source.write_bytes(b"fake elf binary content")

        with patch.object(telegram_routes, "_STAGING_DIR", tmp_path / "staging"), \
        ...
        assert ok is True
        staged = tmp_path / "staging" / "telegram-bot-api"
        assert staged.read_bytes() == b"fake elf binary content"
        mock_cmd.assert_called_once_with(
            ["systemctl", "start", "ahc-telegram-install-binary.service"], timeout=30
        )

    async def test_install_service_unit_stages_and_starts_unit(self, tmp_path):
        """Same pattern as test_install_binary_from_stages_and_starts_unit,
        for the systemd unit file itself."""
        from app.routes import telegram_routes

        with patch.object(telegram_routes, "_STAGING_DIR", tmp_path / "staging"), \
             patch("app.routes.telegram_routes.run_command", new_callable=AsyncMock) as mock_cmd:
            mock_cmd.return_value = (0, "", "")
            ok, err = await telegram_routes._install_service_unit("[Unit]\nDescription=test\n")

        assert ok is True
        staged = tmp_path / "staging" / "telegram-bot-api.service"
        assert staged.read_text() == "[Unit]\nDescription=test\n"
        mock_cmd.assert_called_once_with(
